...
fileDir_toWrite = os.path.join(fileDir, '../rgb_polygon_80/')
...
for file in os.listdir(fileDir):
	if file.endswith(".png"):
		...
		'''
		rectangle_selection = np.array([[10,5],[10,225],[50,225],[50,5]], np.float64)
		rectangle_selection = rectangle
		rectangle_selection[0]=rectangle[0]
		rectangle_selection[1]=rectangle[1]
		rectangle_selection[2]=0.8*rectangle[2]
		rectangle_selection[3]=0.8*rectangle[3]
		'''
		isClosed = False
		color = (128,128,128)
		thickness = 2
		...
		while(True):
			...
			points = np.array([ \
			[(line_in_np[1]-line_in_np[3]*threshold/2)*w,(line_in_np[2]-line_in_np[4]*threshold/2)*h], \
			[(line_in_np[1]-line_in_np[3]*threshold/2)*w,(line_in_np[2]+line_in_np[4]*threshold/2)*h], \
			[(line_in_np[1]+line_in_np[3]*threshold/2)*w,(line_in_np[2]+line_in_np[4]*threshold/2)*h], \
			[(line_in_np[1]+line_in_np[3]*threshold/2)*w,(line_in_np[2]-line_in_np[4]*threshold/2)*h] \
			])
